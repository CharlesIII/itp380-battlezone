    ...
    public class PlayerTank : Actor
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            else
            {
                if (gamePlay)
                {
                    ...
                    if (RotateLeft && !isColliding)
                    {
                        LWheelRotation -= (Delta);
                        RWheelRotation += (Delta);
                        TurretRotation -= ((float)Math.PI / 5) * Delta;
                        RotAngle -= ((float)Math.PI / 5) * Delta;
                        Quat *= Quaternion.CreateFromAxisAngle(new Vector3(0.0f, 1.0f, 0.0f), ((float)Math.PI / 5) * Delta);
                    }
                    else if (RotateRight && !isColliding)
                    {
                        LWheelRotation += (Delta);
                        RWheelRotation -= (Delta);
                        TurretRotation += ((float)Math.PI / 5) * Delta;
                    ...
                    /*
                    else if (RotateLeft && isColliding)
                    {
                        //NOTICE: our left and right are flipped in the model
                        //Console.Out.WriteLine(Vector3.Dot(Velocity, worldTransform.Left));
                        if (Vector3.Dot(Velocity, worldTransform.Left) > 0)
                        {
                            //so if our velocity is going towards the right and we are colliding, then we can't be rotating left
                            Console.Out.WriteLine("Velocity is heading towards the right");
                        }
                        else
                        {
                            
                            //our velocity is heading left so we can rotate left
                            LWheelRotation -= (Delta);
                            RWheelRotation += (Delta);
                            TurretRotation -= ((float)Math.PI / 5) * Delta;
                            RotAngle -= ((float)Math.PI / 5) * Delta;
                            Quat *= Quaternion.CreateFromAxisAngle(new Vector3(0.0f, 1.0f, 0.0f), ((float)Math.PI / 5) * Delta);
                            //Velocity = Velocity + Vector3.Normalize(Velocity) * ((float)Math.PI / 5) * Delta;
                        }
                    }
                    else if (RotateRight && isColliding)
                    {
                        //same thing as above
                        if (Vector3.Dot(Velocity, worldTransform.Right) > 0)
                        {
                            //shouldn't try to rotate right
                        }
                        else
                        {
                            LWheelRotation += (Delta);
        ...
        public override void collide(Actor a)
        {
            ...
            else if (a.COLLISION_IDENTIFIER == CollisionIdentifier.BUILDING)
            {
                ...
                //Console.Out.WriteLine("Setting collision to true");
                Building b = (Building)a;
                ...
                //System.Console.Out.WriteLine(perpVelComp);
                Velocity += perpVelComp;

                //NOTICE: our left and right is flipped in the model
                if (RotateLeft && Vector3.Dot(Velocity, worldTransform.Left) > 0)
                {
                    //undo the rotation done if we're moving right and trying to rotate left
                    RotAngle += ((float)Math.PI / 5) * Delta;
                    Quat *= Quaternion.CreateFromAxisAngle(new Vector3(0.0f, -1.0f, 0.0f), ((float)Math.PI / 5) * Delta);
                }
                else if (RotateRight && Vector3.Dot(Velocity, worldTransform.Right) > 0)
                {
                    RotAngle -= ((float)Math.PI / 5) * Delta;
                    Quat *= Quaternion.CreateFromAxisAngle(new Vector3(0.0f, 1.0f, 0.0f), ((float)Math.PI / 5) * Delta);
                }
            ...
            else if (a.COLLISION_IDENTIFIER == CollisionIdentifier.SHELL)
            {
                ...
                if (a.dead)
                {
                    ...
                    {
                        lives--;
                        GameplayScreen.Instance.m_kLifeCount.life = lives;
                        dead = true;
        ...
        public Plane findIntersectingPlane(Building b)
        {
            ...
            Vector3 firstPlaneCorner1;
            Vector3 firstPlaneCorner2;
            Vector3 secondPlaneCorner1;
            Vector3 secondPlaneCorner2;
            ...
            if (firstClosest.X == secondClosest.X)
            {
                ...
                firstPlaneCorner1 = firstClosest;
                firstPlaneCorner2 = secondClosest;
            }
            else if (firstClosest.X == thirdClosest.X)
            {
                ...
                firstPlaneCorner1 = firstClosest;
                firstPlaneCorner2 = thirdClosest;
            }
            else
            {
                ...
                firstPlaneCorner1 = secondClosest;
                firstPlaneCorner2 = thirdClosest;
            }
            ...
            if (firstClosest.Z == secondClosest.Z)
            {
                ...
                secondPlaneCorner1 = firstClosest;
                secondPlaneCorner2 = secondClosest;
            }
            else if (firstClosest.Z == thirdClosest.Z)
            {
                ...
                secondPlaneCorner1 = firstClosest;
                secondPlaneCorner2 = thirdClosest;
            }
            ...
            else
            {
                ...
                secondPlaneCorner1 = secondClosest;
                secondPlaneCorner2 = thirdClosest;
            }
            ...
            //Console.Out.WriteLine(wall1 + " distance " + distance1);
            //Console.Out.WriteLine(wall2 + " distance " + distance2);

            if (distance1 < distance2)
            {
                if (WorldPosition.Z <= Math.Max(firstPlaneCorner1.Z, firstPlaneCorner2.Z) && WorldPosition.Z >= Math.Min(firstPlaneCorner2.Z, firstPlaneCorner1.Z))
                    return wall1;
                else if (WorldPosition.X <= Math.Max(secondPlaneCorner1.X, secondPlaneCorner2.X) && WorldPosition.X >= Math.Min(secondPlaneCorner1.X, secondPlaneCorner2.X))
                    return wall2;
                else if (WorldPosition.X - WorldBounds.Radius <= Math.Max(secondPlaneCorner1.X, secondPlaneCorner2.X))
                    return wall2;
                else if (WorldPosition.X + WorldBounds.Radius >= Math.Min(secondPlaneCorner2.X, secondPlaneCorner1.X))
                    return wall2;
                else
                    return wall1;
            }
            ...
            {
                if (WorldPosition.X <= Math.Max(secondPlaneCorner1.X, secondPlaneCorner2.X) && WorldPosition.X >= Math.Min(secondPlaneCorner1.X, secondPlaneCorner2.X))
                    return wall2;
                else if (WorldPosition.Z <= Math.Max(firstPlaneCorner1.Z, firstPlaneCorner2.Z) && WorldPosition.Z >= Math.Min(firstPlaneCorner2.Z, firstPlaneCorner1.Z))
                    return wall1;
                else if (WorldPosition.Z - WorldBounds.Radius <= Math.Max(firstPlaneCorner1.Z, firstPlaneCorner2.Z))
                    return wall1;
                else if (WorldPosition.Z + WorldBounds.Radius >= Math.Min(firstPlaneCorner1.Z, firstPlaneCorner2.Z))
                    return wall1;
                else
                    return wall2;
            }
